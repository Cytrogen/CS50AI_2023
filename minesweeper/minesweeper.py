...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        # Mark the cell as a move that has been made and mark the cell as safe
        ...
        for i in range(cell[0]-1, cell[0]+2):
            for j in range(cell[1]-1, cell[1]+2):
                # If the cell is not the cell itself, is not in the safes and is not in the moves_made
                if (i, j) != cell and (i, j) not in self.safes and (i, j) not in self.moves_made:
                    if 0 <= i < self.height and 0 <= j < self.width:
